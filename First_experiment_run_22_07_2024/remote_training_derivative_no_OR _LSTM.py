...
def main(a,b, c):
    ...
    lr_fulldata = 0.0005
    batch_size_no_or_fulldata = 2000
    ...
    params_tcn =    {
                        "window_size" : 30,
                        "learning_rate" : lr,
                        "learning_rate_fulldata" : lr,
                        "batch_size" : batch_size_no_or,
                        "batch_size_fulldata" : batch_size_no_or,
                        "n_hidden" : 5,
                        "levels" : 4,
                        "kernel_size" : 7,
                        "dropout" : 0
    ...
    # Configure logging
    log_file = f"training_no_OR_derivative_LSTM_{c}.log"
    ...
    train_set_lstm = CustomDataset(train_data, window_size=params_lstm["window_size"])
    train_loader_lstm_fulldata = DataLoader(train_set_lstm, batch_size=params_lstm["batch_size_fulldata"], pin_memory=True)
    ...
    for e in tqdm(range(params_tcn["epochs"])):
        ...
        if e < 200:
            ...
            train_lstm_no_or_derivative(train_loader_lstm_fulldata, model_lstm, learning_rate= params_lstm["learning_rate_fulldata"], weight_decay=weight_decay)
        

        # Every few epochs get the error MSE of the true data
        # compared to the network prediction starting from some initial conditions
        if (e+1)%test_every_epochs == 0:
            _, err_test_lstm = test(test_data.to(device), model_lstm, model_type = "lstm_derivative", window_size=params_lstm["window_size"], display_plots=False, num_of_inits = test_n, set_rand_seed=True, physics_rescaling = PSW_max)
            ...
            if err_test_lstm<0.01 and e>200:
                break
