...
def exp1(models: dict, data, test_data, window_size, plot_errs=False, set_random=False):
    ...
    if not plot_errs:
        plot_predictions(predictionary)
    else:
        plot_errors(error_dict)

def plot_predictions(predictionary):
    ...
    for key, pred in predictionary.items():
        ...
        axs[0].plot(time, pred.detach().cpu().numpy()[:, 1], colors=colors[key], label=f"{key}-prediciton")
        axs[1].plot(time, pred.detach().cpu().numpy()[:, 2], color=colors[key], label=f"{key}-prediciton")
