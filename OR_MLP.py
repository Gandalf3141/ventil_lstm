...
def main():
    ...
    for k, params in enumerate(parameter_configs):
        ...
        for e in tqdm(range(params["epochs"])):
            ...
            if (e+1)%50 == 0:
                _,_, err_train = test(test_data.to(device), model, model_type = "or_mlp", window_size=params["window_size"], display_plots=False, num_of_inits = 20, set_rand_seed=True, physics_rescaling = PSW_max)
        ...
        _,_, err_train = test(test_data.to(device), model, model_type = "or_mlp", window_size=params["window_size"], display_plots=False, num_of_inits = 2, set_rand_seed=True, physics_rescaling = PSW_max)
